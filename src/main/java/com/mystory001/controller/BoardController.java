...
public class BoardController {
	...
	public String fwritePro(HttpServletRequest request, MultipartFile file) throws Exception {
		...
		// 파일 업로드 게시판 → 프로그램 설치 → pom.xml(commons-fileupload, commons-io,
		// javax-annotation 설치) → servlet-context.xml에서 프로그램 설정 1) 대용량 데이터베이스(오라클) 컬럼에
		// 저장, 2) 저용량 데이터베이스(MySql) 서버 폴더에 파일 업로드하고 데이터베이스에 파일 이름이 저장
		// name = "file" → MultipartFile → file 이름이 동일. 즉 name="이름"이름과 MultipartFile 이름이
		// 동일해야함
		// 업로드 파일 이름이 동일할 경우 → 랜덤문자_파일이름 으로 이름 변경. 즉 파일이름이 중복되지 않게 업로드 시 랜덤문자를 추가 UUID
		UUID uuid = UUID.randomUUID();
		String fileName = uuid.toString() + "_" + file.getOriginalFilename();
		// 업로드 원본 파일 file.getByte() → upload폴더에 복사(파일 업로드). FileCopyUtils.copy(원본파일,
		// 복사파일);
